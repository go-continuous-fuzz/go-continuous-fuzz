...
// object's metadata.
//
// This is only valid for corpus objects that include the "last-minimized"
// metadata field. Returns current time if the object is missing. Returns error
...
func (s3s *S3Store) getLastMinimizedTime() (time.Time, error) {
	...
	if err != nil {
		var nsk *types.NoSuchKey
		if errors.As(err, &nsk) {
			// Object doesn't exist, sp default to current time
			return time.Now(), nil
		}
	...
	if !ok || lastMinStr == "" {
		return time.Time{}, fmt.Errorf("internal error: missing "+
			"last-minimized metadata for key %q", s3s.zipKey)
	}
